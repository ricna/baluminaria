...
public class AudioManager : Singleton<AudioManager>
{
    ...
    public AudioSource PlaySFX(AudioClip sfxClip, float volume = 1f, float pitch = 1f)
    {
    ...
    public AudioSource PlaySFXStartThenLoop(AudioClip sfxClipStartOnce, AudioClip sfxLoop, float volume = 1f, float pitch = 1f)
    {
        if (sfxClipStartOnce == null || sfxLoop == null) return null;
        AudioSource source = GetAvailableSfxAudioSource();
        if (source != null)
        {
            source.clip = sfxClipStartOnce;
            source.volume = volume;
            source.pitch = pitch;
            source.Play();
            PlayAfterPreviousSFX(source, sfxLoop, volume, pitch);
        }
        return source;
    }

    private void PlayAfterPreviousSFX(AudioSource source, AudioClip nextClip, float volume, float pitch)
    {
        StartCoroutine(PlayNextSFXCoroutine(source, nextClip, volume, pitch));
    }

    private IEnumerator PlayNextSFXCoroutine(AudioSource source, AudioClip nextClip, float volume, float pitch)
    {
    ...
    public void PlayOnceAfterLoop(AudioSource audioSourceBurner)
    {
        StartCoroutine(StopSFXAfterLoopCoroutine(audioSourceBurner));
    }

    private IEnumerator StopSFXAfterLoopCoroutine(AudioSource audioSourceBurner)
    {
        if (audioSourceBurner == null) yield break;
        yield return new WaitWhile(() => audioSourceBurner.isPlaying && !audioSourceBurner.loop);
        yield return new WaitForEndOfFrame();
        StopSFX(audioSourceBurner);
    }
}
